...
class ImageProcessingTransformer(nn.Module):
    ...
    def __init__(self, patch_size=1, in_channels=1, mid_channels=3, num_classes=1000, depth=4,
                 num_heads=4, ffn_ratio=2., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 norm_layer=nn.LayerNorm):
        super(ImageProcessingTransformer, self).__init__()

        self.num_classes = num_classes
        self.embed_dim = patch_size * patch_size * mid_channels
        self.headsets = Head(in_channels, mid_channels)
        ...
        # tail
        self.tailsets = Tail(mid_channels, in_channels)
...
def ipt_base():
    model = ImageProcessingTransformer(
        patch_size=4, depth=4, num_heads=4, ffn_ratio=2, qkv_bias=True,
